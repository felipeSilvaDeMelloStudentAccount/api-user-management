...
import static api.user.management.utils.Constants.MESSAGE;
...
public class UserInformationService {
  ...
  public ResponseEntity<String> getUser(String userid) {
    ...
    try {
      ...
      return new ResponseEntity<>(DATABASE_ERROR + e.getMessage(),
  ...
  public ResponseEntity<String> deleteUser(String userid) throws JsonProcessingException {
    ...
    try {
      ...
      return new ResponseEntity<>(objectMapper.writeValueAsString(map),
          HttpStatus.INTERNAL_SERVER_ERROR);
    } catch (Exception e) {
      log.debug("Error while deleting user: {}", e.getMessage());
      Map<String, String> map = new HashMap<>();
